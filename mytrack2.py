...
from yolov5.utils.general import check_img_size, non_max_suppression,non_max_suppression_cls, scale_coords
...
output ='inference/202208ALL/'
fourcc=cv2.VideoWriter_fourcc(*'mp4v')
conf_thres = 0.1
source='/media/fulab/fulab_cst/CST_bacteria/202208processed/ALL'
...
out, source, weights, view_img, save_txt, imgsz, save_img, sort_max_age, sort_min_hits, sort_iou_thresh = \
    output, source, weights, view_img, save_txt, img_size, save_img, sort_max_age, sort_min_hits, sort_iou_thresh
...
for frame_idx, (path, img, im0s, vid_cap) in enumerate(dataset):  # for every frame
    ...
    predo = model(img, augment=augment)[0]

    # Apply NMS
    pred = non_max_suppression_cls(
        predo, conf_thres, iou_thres, classes=classes, agnostic=agnostic_nms)
    ...
    for i, det in enumerate(pred):  # for each detection in this frame
        ...
        for c in det[:, -2].unique():  # for each unique object category
            n = (det[:, -2] == c).sum()  # number of detections per class
            s += f' - {n} {names[int(c)]}'
        ...
        for x1, y1, x2, y2, conf, detclass,cls_substract in det.cpu().detach().numpy():
            dets_to_sort = np.vstack((dets_to_sort, np.array([x1, y1, x2, y2, conf, detclass,cls_substract])))
        print('\n')
        ...
        if len(tracked_dets) > 0:
            ...
            categories = tracked_dets[:, 5]
            identities = tracked_dets[:, 7]
        ...
        if save_txt and len(tracked_dets) != 0:
            for j, tracked_dets in enumerate(tracked_dets):
                ...
                confident = tracked_dets[4]
                category = tracked_dets[5]
                cls_subs = tracked_dets[6]
                identity = tracked_dets[7]

                with open(txt_path, 'a') as f:
                    f.write(f'{Path(p).name},{frame_idx},{bbox_x1},{bbox_y1},{bbox_x2},{bbox_y2},{category},{confident},{cls_subs},{identity}\n')
...
#                         default='/media/fulab/2T/data/bacteria-avi', help='source')
